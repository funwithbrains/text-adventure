define(['utils'], ({ seedrandom }) => {
  ...
  const createSource = (worldConfig) => {
    ...
    const rootNames = [];
    for (let i = 0; i < 32; ++i) {
      rootNames.push(peopleNameGenerator.create(rng));
    }
    const localGenerator = createMarkovGenerator(rootNames, 3);

    return {
      localGenerator,
      roll: () => {
        return localGenerator.create(rng); // TODO seed this directly for determinism
      }
    };
  };
